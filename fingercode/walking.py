...
import numpy as np
import numpy.matlib
from . import ridge_segment
from . import ridge_orient
...
def walking(img):
	...
	blksze = 16;
	thresh = 0.3;
	normim, mask = ridge_segment.ridge_segment(img,blksze,thresh);    
	mask[(mask.shape[0]//blksze)*blksze+1:mask.shape[0],:] = 0
	mask[:,(mask.shape[1]//blksze)*blksze+1:mask.shape[1]] = 0


	orientim = ridge_orient.ridge_orient(normim, 1, 3, 3)
